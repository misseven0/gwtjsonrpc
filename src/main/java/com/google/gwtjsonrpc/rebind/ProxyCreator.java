...
import com.google.gwtjsonrpc.common.RpcMethod;
import com.google.gwtjsonrpc.common.RpcService;
...
class ProxyCreator
{
	...
	private void checkMethods(final TreeLogger logger) throws UnableToCompleteException
	{
		...
		for (final JMethod m : methodList)
		{
			...
			if (params.length == 0)
			{
				invalid(logger, "Method " + m.getName() + " requires " + AsyncCallback.class.getName() + " as last parameter");
			}

			final JParameter callback = params[params.length - 1];
			if (!callback.getType().getErasedType().getQualifiedSourceName().equals(asyncCallbackClass.getQualifiedSourceName()))
			{
				invalid(logger, "Method " + m.getName() + " requires " + AsyncCallback.class.getName() + " as last parameter");
			}
			...
			if (returnsCallbackHandle(m))
			{
				...
				{
					invalid(logger, "CallbackHandle return value of " + m.getName() + " must have a type parameter");
				}
				if (!resultType.getQualifiedSourceName().equals(rt.isParameterized().getTypeArgs()[0].getQualifiedSourceName()))
				{
					invalid(logger,
							"CallbackHandle return value of " + m.getName() + " must match type with AsyncCallback parameter");
				}
			}
			...
			if (m.getAnnotation(HostPageCache.class) != null)
			{
				...
				{
					invalid(logger, "Method " + m.getName() + " must return void if using " + HostPageCache.class.getName());
				}
				if (params.length != 1)
				{
					invalid(logger, "Method " + m.getName() + " must not accept parameters");
			...
			final TreeLogger branch = logger.branch(TreeLogger.DEBUG,
					m.getName() + ", result " + resultType.getQualifiedSourceName());
	...
	// 创建一个newJsonCall方法,返回JsonCall20HttpGet
	...
	private String getJsonCallClassName(final TreeLogger logger) throws UnableToCompleteException
	{
		...
		logger.log(Type.ERROR, "Unsupported JSON-RPC version and transport "
				+ "combination: Supported are 1.1 over HTTP POST and " + "2.0 over HTTP POST and GET");
		throw new UnableToCompleteException();
	}
	...
	{
		String serviceMethodName = method.getName();
		RpcMethod rpcMethod = method.getAnnotation(RpcMethod.class);
		if (rpcMethod != null && rpcMethod.name() != null && rpcMethod.name() != "")
		{
			serviceMethodName = rpcMethod.name();
		}
		String serviceName = "";
		RpcService rpcService = svcInf.getAnnotation(RpcService.class);
		if (rpcService != null)
		{
		...
		if (serviceName == "")
		{
			serviceName = svcInf.getName();
		}
		if (serviceName != "")
		{
			serviceMethodName = serviceName + "." + serviceMethodName;
		}
		...
		w.print("doInvoke(");
		w.print("\"" + serviceMethodName + "\"");//
//		w.print("\"" + method.getName() + "\"");//serviceName
		w.print(", " + reqDataStr);
		w.print(", ");
		if (resultType.isParameterized() != null)
		{
