...
class SerializerCreator
{
	...
	static
	{
		...
		defaultSerializers.put(java.lang.String.class.getCanonicalName(), JavaLangString_JsonSerializer.class.getCanonicalName());
		...
		parameterizedSerializers.put(java.util.List.class.getCanonicalName(), ListSerializer.class.getCanonicalName());
		parameterizedSerializers.put(java.util.Map.class.getCanonicalName(), ObjectMapSerializer.class.getCanonicalName());
		parameterizedSerializers.put(java.util.Set.class.getCanonicalName(), SetSerializer.class.getCanonicalName());
	}
			...
			if (leafType.isPrimitive() != null || isBoxedPrimitive(leafType))
			{
				if (type.isArray().getRank() != 1)
				{
					logger.log(TreeLogger.ERROR,
							"gwtjsonrpc does not support " + "(de)serializing of multi-dimensional arrays of primitves");
					// To work around this, we would need to generate
					// serializers for
	...
	private boolean isStringMap(final JType t)
	{
		return t.isParameterized() != null && t.getErasedType().isClassOrInterface() != null
				...
				&& t.getErasedType().isClassOrInterface().isAssignableTo(context.getTypeOracle().findType(Map.class.getName()));
	}
	...
	{
		final String qsn = t.getQualifiedSourceName();
		return qsn.equals(Boolean.class.getCanonicalName()) || qsn.equals(Byte.class.getCanonicalName()) || isBoxedCharacter(t)
				|| qsn.equals(Double.class.getCanonicalName()) || qsn.equals(Float.class.getCanonicalName())
				|| qsn.equals(Integer.class.getCanonicalName()) || qsn.equals(Short.class.getCanonicalName());
	}
	...
	private SourceWriter getSourceWriter(final TreeLogger logger, final GeneratorContext ctx)
	{
		...
		if (targetType.isEnum() != null)
		{
			...
			cf.setSuperclass(EnumSerializer.class.getSimpleName() + "<" + targetType.getQualifiedSourceName() + ">");
		...
		} else
		{
			...
			cf.setSuperclass(ObjectSerializer.class.getSimpleName() + "<" + targetType.getQualifiedSourceName() + ">");
		}
	...
	static boolean needsTypeParameter(final JType ft)
	{
		return ft.isArray() != null
				|| (ft.isParameterized() != null && parameterizedSerializers.containsKey(ft.getQualifiedSourceName()));
	}
